...
> **Companion file:** See also `AGENTS.md` (auto-generated by `create-next-app`) for version-drift warnings that apply to all AI coding agents.
...
- **Frontend:**
  - Next.js (latest stable, App Router) — currently 16.x
...
- Lint with `pnpm lint` (runs `eslint .` against the flat config in `eslint.config.mjs`); `next lint` was removed in Next.js 16
